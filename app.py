...
# #     skills = request.args.getlist('skills')  # Skills should be passed as multiple query params
...
# #     recruiter = recruiters_db.get(email)
...
# # @app.route('/devLogin')
# # def devLogin():
# #     return redirect(f"{FRONTEND_URL}/devLogin")

# # @app.route('/devRegister')
# # def devRegister():
# #     return redirect(f"{FRONTEND_URL}/devRegister")
...
@app.route('/recruiterRegister', methods=['POST'])
def recruiterRegister():
    ...
    adhar = data.get("adhar")  # Unique identifier

    # Check if all fields are provided
    if not all([name, email, phone, password, company, adhar]):
        return jsonify({"error": "All fields are required"}), 400

    # Check if recruiter already exists
    if adhar in recruiters_db.recruiters:
        return jsonify({"error": "Recruiter with this Aadhar already exists"}), 409

    # Create new recruiter object
    new_recruiter = RecruiterProfile(name, email, phone, password, company, adhar)

    # Add recruiter to the database
    response = recruiters_db.addUser(new_recruiter)
